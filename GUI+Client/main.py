...
class GetMacAdd():
	...
	def tryGetMac(self):
		...
		try:
			...
			self.supported = True #  Documents whether our mac address collection method is supported
			Logger.debug('tryGetMac: executed arp -a successfully and got ' + result)
			return result
	...
	def coronaCatcherButtonClicked(self):
		...
		if allowedTime < currentTime:
			...
			if (returnVal == -1):
				...
				this.store["statusLabel"]["home"] = "Last checked at " + lastAccess + ", please only check once every 8 hours. Feel free to return at " + allowedTime
			else:
